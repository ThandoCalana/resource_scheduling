...
COMBINED_SHEET = os.getenv("COMBINED_SPREADSHEET_NAME")
...
def email_to_name(email): return " ".join(p.capitalize() for p in email.split("@")[0].split("."))
...
def write_combined_sheet():
    sheet = init_google_sheet(COMBINED_SHEET)
    ...
    for day in weekdays:
        ...
        try:
            ...
            ws = sheet.add_worksheet(title=sheet_name, rows="200", cols=str(len(OUTLOOK_USER_EMAILS)+len(ASSIGNEES_WITH_UNASSIGNED)+1))
...
if __name__ == "__main__":
    ...
    print("Combined Outlook + ClickUp + Harvest sheet written.")
